...
class Multi_stage_loss(keras.losses.Loss):
  """ Multi_stage_loss """
  def __init__(self, loss, n_stage = 7, main_multi = 1, class_weights = None, **kwargs):
  ...
  def call(self, y_true, y_pred):
    ...
    """
    combined_loss = self.loss(y_true, y_pred[:, 0, ...]) * self.main_multi
    for idx in range(self.n_stage-1):
      combined_loss += self.loss(y_true, y_pred[:, idx, ...])
